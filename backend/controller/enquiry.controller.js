...
export const enquiry = async (req, res) => {
  try {
    const { fullName, email, mobile, inquiry, productName } = req.body;

    if (!fullName || !email || !mobile || !inquiry) {
    ...
    const emailSubject = `New Enquiry for ${productName} from ${fullName}`;
    const emailContent = `
       <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
          <div style="background-color: #226930; color: #ffffff; padding: 20px; text-align: center; font-size: 22px; font-weight: bold;">
            Hippocampus Healthcare - New Enquiry For ${productName}
          </div>
          <div style="padding: 20px; background-color: #f8fff8;">
            <p style="font-size: 16px; color: #333; margin-bottom: 12px;">
              <strong style="color: #226930;">Name:</strong> ${fullName}</p>
             <p style="font-size: 16px; color: #333; margin-bottom: 12px;">
                <strong style="color: #226930;">Email:</strong> ${email}</p>
            <p style="font-size: 16px; color: #333; margin-bottom: 12px;">
              <strong style="color: #226930;">Phone:</strong> ${mobile}</p>
            <p style="font-size: 16px; color: #333; margin-bottom: 12px;">
              <strong style="color: #226930;">Product/Investigation:</strong> ${productName}</p>
            <p style="font-size: 16px; color: #333; margin-bottom: 12px;">
              <strong style="color: #226930;">Enquiry:</strong> ${inquiry}</p>
    ...
    await sendEmail("frontend@jain.software", emailSubject, emailContent);
